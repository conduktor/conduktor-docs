...
- `KUBERNETES` Vault authentication now gets the JWT from the default Vault location (`/var/run/secrets/kubernetes.io/serviceaccount/token`). You can configure this location using `path` (`VAULT_KUBERNETES_PATH`). The `jwt` configuration key is no longer needed. See **Kubernetes authentication** under [Vault authentication types](/guide/reference/interceptor-reference#vault-kms).
