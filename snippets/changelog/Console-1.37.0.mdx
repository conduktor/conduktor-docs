...
- [Breaking changes](#breaking-changes)
  - [Gateway 3.12 dependency](gateway-3.12-dependency)
  - [Email address storage](email-address-storage)
...
- [Conduktor Trust](#conduktor-trust)
  ...
  - [Data quality rule breaking change](#data-quality-rule-breaking-change)
...
Starting from Console 1.37, Conduktor Trust [Mark action](#mark-action) and Conduktor Exchange [Partner Zone](#partner-zone-breaking-change) functionality will require Gateway 3.12 or later.
...
With the support for multi-cluster Gateway in 3.12, all existing Partner Zones created prior to this version of Console are incompatible and cannot be migrated or updated to support the new capability. Hence, existing Partner Zones will stop functioning after the upgrade if not recreated.
...
#### Data quality rule breaking change

With the introduction of **Mark action** the `/public/data-quality/v1/data-quality-rule` has a breaking change on `PUT` where it now needs `mark` data passed in the payload. See [API documentation](https://developers.conduktor.io/?product=console&version=1.37.0#tag/cli_data-quality-rule_data-quality_v1_17/operation/list-data-quality-rule-resources) for details.
...
- Fixed "404 group does not exist" error incorrectly showing up when upgrading from 1.35 to version 1.36.
...
- Improved groups API performance, preventing timeouts for big organisations with thousands of users and groups.
- Improved indexer performance, preventing timeouts for big organisations with thousands of consumer groups.
- JVM metrics renamed in the Console and the Grafana dashboard, based on [Prometheus changes](https://prometheus.github.io/client_java/migration/simpleclient/#jvm-metrics).
