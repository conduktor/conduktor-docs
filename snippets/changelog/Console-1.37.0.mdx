...
In this release, we have made a change to how we store the email addresses of users in Console. All emails are now stored in lowercase to ensure consistency and prevent issues with SSO or RBAC. We highly recommend that you back up your Console database before upgrading to this version. If two users have the same email address but with different cases, the migration will fail. In this case, we recommend that you resolve the conflict by either deleting one of the users or changing their email address before upgrading.
...
- Fixed password field length disclosure in configuration forms, now showing consistent obfuscation regardless of actual password length.
- Fixed secrets being displayed in clear text in AWS Glue and other cluster configuration forms.
...
- Fixed schema name truncation in the reprocessing topic messages flow, now displaying complete schema names even for long names.
- Fixed connector error messages not being displayed when connectors fail before task creation, improving visibility into connector failures.
...
- Fixed group creation timeouts when using REST APIs with large user and group counts, improving API performance for organizations with thousands of users.
- Improved Console performance for large-scale deployments, addressing timeout issues with consumer group indexing and topic details.
- Fixed metrics displaying incorrect values on the home page overview after upgrade from 1.31.0 to 1.31.1.
