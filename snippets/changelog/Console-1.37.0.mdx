...
New application group management in Self-Service provides full lifecycle management through improved UI workflows. The **members tab** offers an enhanced interface for managing group membership, making it easier for teams to control access and visibility. The **resources tab** enables granular control over topic, consumer group, subject, and connector permissions for precise access management. Application groups can also map to external groups like console user groups for streamlined access management. 

#### Application Instance Resources

Application instances now feature comprehensive resource management capabilities. The **new Resources tab** provides a detailed view of all resources associated with application instances, giving teams better visibility into their resource usage and dependencies.
...
The self-service experience has been improved with **updated terminology** throughout the application for better clarity, access request workflows now feature clearer language and better user experience.
...
- Text overflow issues with certain strings in application group interfaces have been resolved.
- Schema name display improvements allow users to see full schema names without truncation.
- Alert form improvements include making webhook body optional to prevent silent alert failures and adding default email body values.
