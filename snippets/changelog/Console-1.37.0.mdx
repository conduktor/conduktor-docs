...
- In the reprocessing topic messages flow schema names are no longer truncated.  We now display complete schema names even for long names.
- Connectors which failed before task creation will now show an error message, giving increased visibility into connector failures.
