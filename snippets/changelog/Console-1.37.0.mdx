...
- Fixed schema name truncation in the UI, now displaying complete schema names even for long names.
- Display connector failure reasons when it fails during initialisation, not only when tasks fail.
- Fixed user and group API endpoints returning errors when referenced clusters are deleted, now handling deleted cluster references gracefully.
- Fixed error incorrectly showing up when upgrading Conduktor Console from 1.35.0 to version 1.36.
