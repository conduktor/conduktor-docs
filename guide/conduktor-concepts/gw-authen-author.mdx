...
Gateway supports different client authentication methods, depending on both the type of [service account](/guide/conduktor-concepts/gw-service-accounts)(local or external) and the security mode ([Gateway managed](#in-gateway-managed-mode) or [Kafka managed](#in-kafka-managed-mode)). See the table below as a guide for which service account will need to be provided.
