---
title: Data security Interceptors
---

You can:

- [audit API key requests for compliance tracking](#audit-interceptor) 
- [mask sensitive data](#data-masking-interceptor) in messages during consumption
- [encrypt data](#encryption-interceptors) when producing or consuming. For example, encrypt:
    - specific record fields or entire payloads on consume
    - payload fields configured in specified schemas on consume
    - specific record fields or entire payloads on produce
    - payload fields configured in specified schemas on produce
- [decrypt](#decryption-configuration) any record fields encrypted by Gateway or entire payloads on consume
- [inject dynamic headers](#dynamic-header-injection-interceptor) into messages during production
