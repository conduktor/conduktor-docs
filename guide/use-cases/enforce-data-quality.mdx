...
Data quality rules define the logic for validating your Kafka messages. There are several types of rules you can use:

- [CEL Expression Rules](#cel-expression-rules)
- [Built-in Rules](#built-in-rules)
- [JSON Schema Rule](#json-schema-rule)
...
<Tabs>
  <Tab title="Console UI">
    ...
    - Common Expression Language (CEL) is an expression language supporting common operators like `==` and `>` as well as macros like `has()` to check for the presence of fields. Use `matches()` to test regex patterns. See [the CEL language definition](https://github.com/google/cel-spec/blob/master/doc/intro.md) for details.
    - (Optional) Click **Show regex library** to access pre-built regex examples. You can click on the required one(s) to copy them and paste into your Rule expression to customize further, as necessary.
    - For JSON Schema, enter your schema definition. See [the JSON Schema Specification](https://json-schema.org/specification) for details.
    1. Click **Create**.
  </Tab>
...
Here are some examples of data quality rules using CEL expressions.
...
- Must follow the [JSON Schema Specification](https://json-schema.org/specification)
- If no `$schema` is specified, defaults to [Draft 2020-12](https://json-schema.org/draft/2020-12)
...
- **Mark**: when a violation occurs, add a special header to the message so it can be identified and handled downstream (see [How Mark Works](#how-mark-works)).
...
<Info>
If both **Mark** and **Block** are enabled for a policy, only blocking will occur. Messages violating policies will be blocked entirely with no marking.
</Info>
#### How Mark Works

When the **Mark** action is enabled, messages that violate data quality rules are tagged with a special header.

- **Header Name:** `conduktor.dataquality.violations`
- **Header Value:** JSON object mapping policy names to arrays of failed rule names
