...
Each Partner Zone can have one of these statuses:

- **Pending**: the zone is being set up (this usually takes a few moments).
- **Ready**: the zone is ready and partners can connect and access it.
- **Failed**: something went wrong, [check out troubleshoot](#troubleshoot).
- **To delete**: the zone is being removed and shouldn't be used/shared.
- **Deleted**: the zone has been completely removed.
...
3. Click on a Partner Zone to see the status.
...
1. In Console, go to **Monitoring** > **Alerts**.
2. Click **Create Alert**.
3. Choose:
   - **Type**: Partner Zone
   - **Condition**: status equals Failed
   - **Notification**: email, Slack, or Teams (as required)

<Note>
To ensure your partners don't lose access, we recommend checking Partner Zone status before sharing the credentials and each time you make changes to Gateway or shared topics.
</Note>

## Troubleshoot

<AccordionGroup>
<Accordion title="I'm getting 'Failed' status, what could be the issue?">
    <p>
    If you get **Failed to connect to Gateway cluster**:
  ...
  - [use the API](https://developers.conduktor.io/?product=console&version=1.31.2#tag/cli_partner-zone_console_v2_16/operation/get-partner-zone-by-name) to GET the state of the Partner Zone.
  - Check Gateway logs:
  ...
  - Check Console logs:
      ...
      ```
  - Test Gateway connection manually to see if Gateway is responding:
      ```bash
