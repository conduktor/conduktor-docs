...
1. Save this example to file, e.g. `pz.yaml`:
    ...
    <Warning>
    As of Console 1.37.0, the field `cluster` has been replaced with `gatewayClusterId`. 
    </Warning>

    * If your Gateway is [setup with multi-clusters](/guide/conduktor-in-production/deploy-artifacts/deploy-gateway/index#configure-gateway-for-multi-clusters), `gatewayClusterId` will be the technical cluster Id of the Gateway as before, and `sourceClusterId` will be the cluster Id of the underlying cluster configured under the Gateway.
