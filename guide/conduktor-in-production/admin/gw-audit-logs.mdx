...
- `"{"status":"invalid_token"}"` - Gateway is unable to decode the OAUTHBEARER token. Can be caused by a Gateway misconfiguration (e.g. wrong issuer URL, client ID, or secret)
Note: Authentication failure can happen when the kafka client fails to fetch the OAUTHBEARER token from the configured OAUTHBEARER provider. This is not a Gateway issue, but rather a misconfiguration of the OAUTH2 provider or the client. The OAUTH2 provider may have specific error messages that can help diagnose the issue.
