...
- `"Authentication failed: credentials for user could not be verified. Cause: Password token is valid but username <username> is invalid"` - Valid token but incorrect username
...
- `"Authentication failed: credentials for user could not be verified. Cause: Password token is valid for username, but username <username> is not an existing local user."` - No local service account defined

**Failed Authentication - Gateway Managed - OAUTHBEARER:**
- `"{"status":"invalid_token"}"` - Invalid JWT token
Note: Authentication may also fail if the Kafka client is unable to fetch the token from the configured OAUTH provider. This occurs before reaching the Gateway, so no audit log event would be generated.
