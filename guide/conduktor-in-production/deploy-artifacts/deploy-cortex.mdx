...
| `PROMETHEUS_RETENTION_SIZE`                   | Prometheus `storage.tsdb.retention.size` startup argument that define retention size                     | false     | string | `5MB`                  | `1.37.0` |
| `PROMETHEUS_RETENTION_TIME`                   | Prometheus `storage.tsdb.retention.time` startup argument that define retention time                     | false     | string | `30m`                  | `1.37.0` |
| `PROMETHEUS_BLOCK_DURATION`                   | Prometheus `storage.tsdb.min-block-duration` and `storage.tsdb.max-block-duration` startup arguments format "min,max"    | false     | string | `5m,30m`               | `1.37.0` |
...
| `CORTEX_CLEANER_INTERVAL`                     | Configure how often cleanup task is running (only use when no external object storage configured)        | false     | string | `6h`                   | `1.37.0` |
| `CORTEX_CLEANER_RETENTION`                    | Configure the retention duration we want in "file system bucket" (`/var/conduktor/monitoring/data/cortex/tsdb-backend/`).  | false     | string | `30d`                   | `1.37.0` |
...
| `PROMETHEUS_EXTRA_RUN_ARGS`                   | Extra Prometheus run arguments                                                                           | false     | string | `""`     | `1.37.0` |
| `CORTEX_EXTRA_RUN_ARGS`                       | Extra Cortex run arguments                                                                               | false     | string | `""`     | `1.37.0` |
| `CORTEX_ALERT_EXTRA_RUN_ARGS`                 | Extra Cortex Alert manager run arguments                                                                 | false     | string | `""`     | `1.37.0` |
...
Cortex Alert manager configuration can also be **patched** by mounting a YAML file into path `/opt/override-configs/alertmanager.yaml`. For an alternative path set the location using the environment variable `ALERTMANAGER_OVERRIDE_CONFIG_FILE`. 
...
**Prometheus**
Prometheus configuration can be also **patched**, by mounting a YAML file into path `/opt/override-configs/prometheus.yaml`. For an alternative path set the location using an environment variable `PROMETHEUS_OVERRIDE_CONFIG_FILE`.

<Note>
Before Conduktor Console **1.37.0** Prometheus configuration was overridden using **replace** strategy. Since **1.37.0** it was changed to **patch** (YAML merging) strategy.
</Note>
