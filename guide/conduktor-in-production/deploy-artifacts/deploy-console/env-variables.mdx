...
You can choose to use your own solution via the `monitoring.backend` property. We support Cortex, Mimir and Prometheus.

You can also choose to not deploy any monitoring solution. However, you will then not be able to see the monitoring graphs or configure alerts.

Depending on the chosen solution, you will need to provide the corresponding URL and other properties. The mandatory column indicates whether the property is required for the chosen backend.
For example, if you choose `mimir`, you must provide the `monitoring.mimir-url` property, but if you choose `conduktor`, you must provide the `monitoring.cortex-url` property.

In case you do not provide an alert manager URL, the Console will use its built-in alert system.
...
| `monitoring.repeat-alert-frequency-minutes` | Frequency in minutes of the repetition of firing alerts (builtin alert system)                     | `CDK_MONITORING_REPEATALERTFREQUENCYMINUTES` | No                                              | int                                                         | `60`          |
| `monitoring.check-alert-frequency-minutes`  | Frequency in minutes of the verification of alert status (builtin alert system)                    | `CDK_MONITORING_CHECKALERTFREQUENCYMINUTES`  | No                                              | int                                                         | `1`           |
| `monitoring.check-alert-parallelism`        | How many alerts are verified in parallel (builtin alert system)                                    | `CDK_MONITORING_CHECKALERTPARALLELISM`       | No                                              | int                                                         | `5`           |
