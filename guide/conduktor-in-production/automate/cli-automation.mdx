...
Conduktor CLI (Command Line Interface) helps you managing your [Conduktor Console and Gateway resources](/guide/reference) via commands or CI/CD pipelines.
...
On the other hand, you can use the external authentication mode to allow you to bypass token generation and use credentials from your identity provider or API proxy instead. For that, set the `CDK_AUTH_MODE` environment variable to `external`:

```bash title="Set up external authentication with a bearer token"
export CDK_AUTH_MODE="external"
export CDK_API_KEY=<bearer_token>
```

```bash title="Set up external authentication with client credentials"
export CDK_AUTH_MODE="external"
...
```bash title="Generate a service account password for a virtual cluster"
conduktor run generateServiceAccountToken --v-cluster my-vcluster --username my-service-account
``` 

```bash title="Generate a Partner Zone service account password"
conduktor run partnerZoneGenerateCredentials --partner-zone-name my-partner-zone
```

```bash title="Get information about the credentials configured in the CLI"
