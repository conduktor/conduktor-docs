...
If you cannot access your database or the Console application, you may need to add inbound rules. For example, in the **security group** attached to the console service, ensure there is an inbound rule (TCP) for port **8080**. In the **security group** attached to the RDS instance, ensure an inbound rule (Type = PostgreSQL) for port **5432**.
