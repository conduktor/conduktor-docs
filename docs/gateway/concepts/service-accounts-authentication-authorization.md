...
- **Local Service Account**: An identity created and managed using the Gateway admin API, with no external dependency. Gateway provides a means to generate Service Account password with a configurable time-to-live. These credentials can then be shared with clients for authentication. Useful if you want to manage the clients credentials directly within Gateway (non delegated client authentication), or if you're using Virtual Clusters.
- **External Service Account**: An identity managed by an external identity provider, such as an OIDC identity provider or an mTLS certificate. This identity can be given a more user-friendly name in the Gateway, though this is optional. For instance, if Azure is used as the OIDC provider, the application principal recognized by the Gateway defaults to a UUID generated by Azure. To enhance readability, this principal can be mapped to a Gateway external Service Account, allowing the use of a friendly name when declaring ACLs, Interceptors, and Audit Logs within the Gateway.
