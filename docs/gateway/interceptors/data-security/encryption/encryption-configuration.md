...
| `compressionType`                                                             | Enum                             | none            | The data is compressed before encryption (only for data configured with full payload encryption). Available values are: `gzip`, `snappy`, `lz4` or `zstd`.|
