...
- These **encryption keys** are stored in a dedicated Kafka topic. They are secure because they cannot be decrypted without the external KMS.
- Overwriting an **encryption key** in the compacted Kafka topic makes the associated encrypted data permanently undecryptable via Gateway.
...
3. crypto shredding keys such that the data can no longer be decrypted by Gateway
...
- Each record will have an encryption key derived from the record value `userId` using Gateway templates. For example the sample JSON from earlier would have the id `secret-for-12345678`.
...
Make a note of the UUID returned as we will need it for crypto shredding the key in step 7: [Crypto shredding](#7-crypto-shredding).
...
The record key is composed of the unique `keyId` that we expected, the cryptographic `algorithm` from the configuration and a `uuid` that is uniquely generated per inserted record. The `uuid` is necessary because it is possible for two different gateway nodes to process a record with the same `keyId` for the first time at the same time. In this scenario two records with the same `keyId` but different a `uuid` would be created. This means that it is possible (although relatively rare) for the same `keyId` to have multiple **encryption keys**.

The **encryption key** in the record value is encrypted with the KMS encryption key and is unusable without it.
...
As before interceptor configuration can be applied to Gateway via the command line or via the Console GUI. 
...
With a decryption interceptor configured on consumption from the same topic we have been encrypting data produced to to it is now possible to decrypt the records we produced and observed as encrypted earlier.

<Tabs>
<TabItem value="Crypto shred via CLI" label="Crypto shred via CLI">
...
<TabItem value="Crypto shred via GUI" label="Crypto shred via GUI">
</TabItem>
</Tabs>
...
The returned records will almost certainly still contain the original **encryption key** that we crypto shredded. This means that associated encrypted data is technically still manually recoverable to anyone with access to the underlying topic and the KMS master key. The key should eventually be deleted because the topic is configured for compaction, but there is no guarantee for when that will occur. Reducing the time until (encrypted) encryption keys are deleted from disk is beyond the scope of this HOWTO. The important thing to remember is that Gateway (and therefore users) will be instantly unable to decrypt data that has been crypto shredded, as the next section will demonstrate.
