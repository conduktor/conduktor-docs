...
- `SAFEGUARD` for events in [Data Quality Producer interceptors](/gateway/category/data-quality/) and [Traffic Control Policies interceptors](/gateway/category/traffic-control-policies/)
- `APIKEYS_REQUEST` for events in [Audit interceptor](/gateway/interceptors/data-security/audit/)
- `REST_API` for all REST v1 admin requests and all REST v2 requests (see [API Reference](/gateway/reference/api-reference/))
- `INTERCEPTOR` for error events in [Encryption interceptors](/gateway/category/encryption/)
