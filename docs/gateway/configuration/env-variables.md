...
| `GATEWAY_MIN_BROKERID`           | `0`                                   | The broker id associated to Gateway's first port (`GATEWAY_PORT_START`). Should be the lowest `broker.id` (or `node.id`) defined in the Kafka cluster. Useful when connecting to Kafka clusters using a specific scheme for allocating broker ids, e.g. if sequential and stable                                                    |
