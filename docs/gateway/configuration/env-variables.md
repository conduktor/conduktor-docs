...
| `GATEWAY_USER_POOL_SERVICE_ACCOUNT_REQUIRED` | If true, verify the existence of user mapping for the service account when the user connects in Non-Delegated SASL/PLAIN mode.  **Note that this variable is deprecated in Gateway v3.9.0**. The behaviour is the same as if it was set to `true` and therefore will verify the existence of user mapping for the service account when the user connects in non-delegated SASL/PLAIN mode.                                                            | `false`       |
