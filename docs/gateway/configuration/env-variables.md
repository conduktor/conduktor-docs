...
| `GATEWAY_AUTO_CREATE_TOPICS_ENABLED`              | Enable auto-create topics feature. When enabled, topics can be automatically created when producing or consuming through the Gateway, leveraging the Kafka property auto.create.topics.enable. **Authorization**: When enabled, users require either `CLUSTER` resource with `CREATE` operation permission (allows creating any topic) or `TOPIC` resource with `CREATE` operation permission for specific topics. **Note**: This feature does not support concentrated topics. When auto-create topics is enabled, topics that would normally be concentrated will be created as physical topics instead. | `false`                                                                                                                                             |
