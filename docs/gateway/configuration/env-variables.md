...
When importing from a file, mount the file into the container and provide its path by setting the environment variable `CDK_ENV_FILE`. The logs will confirm, `Sourcing environment variables from $CDK_ENV_FILE`.
