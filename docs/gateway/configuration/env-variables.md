...
| `GATEWAY_MIN_BROKERID`           | `0`                                   | Broker id associated to gateway first port (`GATEWAY_PORT_START`). Should be the lowest `broker.id` (or `node.id`) defined in the kafka cluster (useful when connecting to kafka clusters using a specific scheme for allocating broker id)                                                     |
